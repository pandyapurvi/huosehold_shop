...
class MyApp extends App {
  static async getInitialProps({ Component, ctx }) {
    ...
    if (!token) {
      ...
      try {
        ...
        const isRoot = user.root === 'root'
        const isAdmin = user.admin === 'admin'
        // if authenticated, but not 'root' or 'admin' we would like to redirect from '/create' page
  ...
  syncLogout = event => {
    if (event.key === 'logout') {
      console.log("Logged out from the local storage")
      Router.push('/logout')
    }
  }
